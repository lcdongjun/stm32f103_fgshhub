...
void OLED_ShowFPS(void)
{
    ...
    if (HAL_GetTick() - lastTick >= 1000) // 1秒过去了
    {
        fps = frameCount;
        frameCount = 0;
        lastTick = HAL_GetTick();

        OLED_ShowString(0, 0, "FPS:", 12, 1);  // 显示前缀
        OLED_ShowNum(24, 0, fps, 3, 12, 1);       // 显示FPS值
    }
}
...
int main(void)
{
  ...
  while (1)
  {
			uint32_t frame_start = HAL_GetTick();  // 记录帧开始时间
			...
			uint32_t frame_end = HAL_GetTick();  // 记录帧结束时间
			uint32_t frame_time = frame_end - frame_start;

			if (frame_time < FRAME_TARGET_MS)
			{
					HAL_Delay(FRAME_TARGET_MS - frame_time);  // 动态补偿延时
			}
//		HAL_Delay(1);
    /* USER CODE END WHILE */
...
//		OLED_ShowChinese(x+17,y,12,16,1);
    ...
    OLED_ShowNum(x+28,y+4, minutes, 2, size, mode); // ��ʾ���ӣ�2λ��
    OLED_ShowString(x + size * 2 * 1-16+28, y-1+4, ":", size, mode); // ��ʾð�ţ�6 ���ַ����ϵ��
    OLED_ShowNum(x + size * 2 * 1-8+28, y+4, seconds, 2, size, mode); // ��ʾ�루2λ��
}
void OLED_ShowEncoderValue(void)
{
