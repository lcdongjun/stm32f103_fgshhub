...
{	
    ...
    HAL_SPI_Transmit(&hspi1, &dat, 1, HAL_MAX_DELAY);  // SPI发送一个字节
...
void OLED_DrawPoint(u8 x,u8 y,u8 t)
{
    ...
    if (t)
        OLED_GRAM[index] |= (1 << bit_offset);  // 置位
    else
        OLED_GRAM[index] &= ~(1 << bit_offset); // 清零
}
...
void OLED_ScrollDisplay(u8 num,u8 space,u8 mode)
{
    ...
    while (1)
    {
        if (m == 0)
        {
            OLED_ShowChinese(128, 24, t, 16, mode);  // 在GRAM中写入下一个字（靠右）
            t++;
        }
        ...
        if (t == num)
        {
            ...
            for (r = 0; r < 16 * space; r++)
            {
                ...
                for (uint16_t page = 0; page < 8; page++)
                {
                    ...
                    OLED_GRAM[page * 128 + 127] = 0x00;  // 最右列清空
                }
                OLED_Refresh();
            }
            t = 0;
